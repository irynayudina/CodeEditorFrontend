 ...
 * @param {Function} fetchFn - Function to fetch data, receives page number and returns { data, totalPages }
 * @param {Object} filterObj - Filter object that triggers reset when changed
 * @param {Object} options - Additional options
 * @returns {Object} - { items, loading, hasNextPage, error, loadMore, reset }
 */
export const useInfiniteScroll = (fetchFn, filterObj = {}, options = {}) => {
  ...
  useEffect(() => {
    const filterChanged = Object.keys(filterObj).length > 0;
    if (filterChanged) {
